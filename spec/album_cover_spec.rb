RSpec.describe 'HTML Album Cover' do
    ...
    context 'within <body>' do
      it 'the title-box section contains the correct header sizes and text content' do
        ...
        expect(title_box.children.select {|ch| ch.name == "h4"}[1].children.first.text).to include("FROM DESPICABLE ME"), "A second 'h4' tag should contain the text 'FROM DESPICABLE ME 2'"
      ...
      it 'the back section contains the correct header sizes and text content' do
        ...
        expect(back_cover.children.select {|ch| ch.name == "h3"}[0].children.first.text).to include("FROM DESPICABLE ME"), "An 'h3' tag should contain the text 'FROM DESPICABLE ME 2'"
